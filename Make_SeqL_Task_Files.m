% File generates item and conditions files for the Sequence Task with
% Intervening images in the List Style (SeqL). Modified from
% Make_ListSQ_Task_Files.m on March 21, 2015 by Seth Konig. 

% you can find the section you are looking for by searching [#]
% 1. Generate blank item file
% 2. Generate blank condition file
% 3 Generate item and condition files with 2 predictable sequences
...
total_items = 43; %number of items in task
...
total_cond = 261; %total number of conditions in task
...
set_nums = 1:40;%always start from 7 otherwise won't regenerate perfectly
number_of_sequences = 2;
number_of_items = 4; %# of items per sequence
number_sequence_trials_btwn_images = 4;
number_of_images = 24;
image_spacing = 8; %number of images between novel and repeat presentations
...
if familiarization_block
    max_sequence_conditions = 192+number_of_sequences*familiar_trials;
else
    max_sequence_conditions = 192;
end
...
rand('seed',150321); %seed randomly so we can regenerate files if lost
for i = 1:500;
...
for itm = set_nums
    ...
    if itm < 10
        set = ['SeqL0' num2str(itm)];
    else
        set = ['SeqL' num2str(itm)];
    end
    fid = fopen([set '.itm'],'w+');
    ...
    for img = 1:number_of_images;
        ...
        str = [itmspace num2str(itmnum)...
            '    8           0.00    0.00      0                                  '...
            '75  75  75 x   C:\\SeqL' setzero num2str(itm) ...
...
root_dir = 'C:\Users\seth.koenig\Documents\MATLAB\Sequence Task_RelationalMemory\';
unused_dir = 'Unused\';
used_dir ='Used\';

cd([root_dir unused_dir]);
setnums = [1:40]; %do not write over original sets!!! Start at 1 will skip ones already created if in root_dir
num_images_per_set = 24;
for set = setnums;
    d=dir([root_dir unused_dir '*.bmp']);
    if set < 10
         set_dir = [root_dir 'SeqL0' num2str(set) '\']; %shortened from ListSQ or directory name too long for cortex
    else
        set_dir = [root_dir 'SeqL' num2str(set) '\']; %shortened from ListSQ or directory name too long for cortex
    end
